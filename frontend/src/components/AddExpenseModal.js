...
function AddExpenseModal({ open, onClose, onAdded }) {
  ...
  return (
    <Modal open={open} onClose={onClose}>
      <Box sx={{
        position:'absolute', top:'50%', left:'50%', transform:'translate(-50%, -50%)', width:300
      }}>
        <Paper sx={{ p:3 }}>
          <Typography variant="h6" mb={2} sx={{ fontWeight:600 }}>Add Expense</Typography>
          ...
          <Button variant="contained" fullWidth onClick={handleAdd}>Add Expense</Button>
        </Paper>
      </Box>
    </Modal>
  );
